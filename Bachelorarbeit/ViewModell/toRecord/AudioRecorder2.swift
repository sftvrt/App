...
class AudioRecorder:  NSObject, ObservableObject {
    ...
    func startRecord(){
        ...
        if nearbyService.isBrowser {
            //audioFileName =  getDocumentsDirectory().appendingPathComponent("\(nearbyService.sitzung.title)+\(dateString).m4a")
            audioFileName =  getDocumentsDirectory().appendingPathComponent("\(nearbyService.sitzung.title) \(dateString).wav")
            
        } else {
            //audioFileName = getDocumentsDirectory().appendingPathComponent("\(nearbyService.peerID.displayName)+\(dateString).m4a")
...
//            nearbyService.sendFile(url: current.fileURL)
//        }
    ...
    func fetchRecordings(){
        DispatchQueue.main.async {
            ...
            print("fetchRecordings.... \(self.recordings) ")
...
extension AudioRecorder {
 ...
 //   func syntheticAudio(_ audioSyntheticType:AudioSyntheticType = AudioSyntheticType.normal) {
    func syntheticAudio(_ audioSyntheticType:AudioSyntheticType = AudioSyntheticType.noConvert) {
